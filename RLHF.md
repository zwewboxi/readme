...
| 参数 | 具体含义 |
|:---: |:--- |
| $r_t(\theta)$ | 概率比率，$r_t(\theta) = \frac{\pi_\theta(a_t|s_t)}{\pi_{old}(a_t|s_t)}$，表示新策略选该动作的概率相对旧策略的变化倍数 |
| $A_t$ | 优势函数，$A_t = Q(s_t, a_t) - V(s_t)$，衡量「选动作$a_t$的价值」与「当前状态平均价值」的差值 |
...
$$
L^{GRPO}(\theta) = \mathbb{E}_{(x,y_1,y_2)\sim \mathcal{D}_{pref}} \left[ \log \sigma \left( \beta \left( \log \frac{\pi_\theta(y_1|x)}{\pi_\theta(y_2|x)} \right) \right) \right] + \mathbb{E}_{(s,a)\sim \mathcal{D}_{rl}} \left[ \min\left( r_t(\theta) A_t, \text{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon) A_t \right) \right] - \lambda D_{KL}(\pi_\theta \parallel \pi_g)
$$
