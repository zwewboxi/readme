...
- **Patch 大小**：假设使用 14×14（SigLIP 常见配置）。
  ...
  - 总 Patch 数：$N = N_h × N_w \approx 89 × 670 \approx 59,630$
- **线性投影**：每个 14×14×3 的 Patch 通过一个可学习的投影矩阵 $W_p \in \mathbb{R}^{(14×14×3) \times D}$ 映射为 $D$ 维向量（SigLIP‑2 中 $D = 768$ 或 1024）。
...
- 输入特征图尺寸：89 × 670（对应 $N_h \times N_w$）。
- 2×2 池化（平均或线性投影）后尺寸：44 × 335 ⇒ 总视觉 token 数 $N_{\text{visual}} = 44 \times 335 \approx 14,740$。
