...
- **Patch 大小**：假设使用 14×14（SigLIP 常见配置）。
  - 垂直 Patch 数：\( N_h = \frac{H}{14} = \frac{1248}{14} ≈ 89 \)
  - 水平 Patch 数：\( N_w = \frac{W}{14} = \frac{9376}{14} ≈ 670 \)
  - 总 Patch 数：\( N = N_h × N_w ≈ 89 × 670 ≈ 59,630 \)
- **线性投影**：每个 14×14×3 的 Patch 通过一个可学习的投影矩阵 \( W_p \in \mathbb{R}^{(14×14×3) × D} \) 映射为 \( D \) 维向量（SigLIP‑2 中 \( D = 768 \) 或 1024）。
...
- 2×2 池化（平均或线性投影）后尺寸：44 × 335 ⇒ 总视觉 token 数 \( N_{\text{visual}} = 44 × 335 ≈ 14,740 \)。
...
| 线性投影 | (59,630, 588) | \( Z = X W_p \) | (59,630, D) | \( W_p \in \mathbb{R}^{588×D} \) |
...
| 时间戳插入 | \( (N_f×M, D') \) | 插入文本 token | \( (N_f×M + N_{\text{ts}}, D') \) | 文本嵌入查找 |
...
1. **动态分辨率适应**：通过 2D‑RoPE 插值，支持任意高宽比的输入图像。
2. **DeepStack 多级融合**：利用不同层的视觉特征，提升细粒度理解能力。
3. **交错 MRoPE**：改进位置编码频谱分布，增强长视频时空建模。
4. **显式时间戳**：用文本 token 表示时间，简化时间建模并提升泛化性。
5. **长上下文支持**：通过分阶段训练（8K → 32K → 256K），使模型能处理长达 256K token 的交错图文视频序列。
6. **计算效率优化**：通过 2×2 特征压缩、MoE 架构等，在保持性能的同时降低计算负担。

这些设计使 Qwen3‑VL 不仅能处理超高分辨率图像（如 9376×1248），还能对长视频进行时空推理，适用于文档解析、视频摘要、多图推理等复杂多模态任务。
